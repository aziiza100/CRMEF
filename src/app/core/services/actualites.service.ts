...
// Mock data removed as per request to only use backend
...
export class ActualitesService {
  ...
  loadActualites(): void {
    ...
    this.api.getActualites().pipe(
      ...
      catchError(err => {
        console.warn('[ActualitesService] API unavailable:', err.message);
   ...
   * Get actualite by ID
   */
  getById(id: number | string): Actualite | undefined {
    const numId = typeof id === 'string' ? parseInt(id, 10) : id;
    return this.actualites().find(a => a.id === numId);
  }
